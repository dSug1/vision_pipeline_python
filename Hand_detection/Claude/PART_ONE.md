...
| B4 | **3.3 A/B — three arms, using blocks** | decision | ⛔ **BUILT 3D-NATIVE, VERIFIED, MEASURED LIVE — DISQUALIFIED AS DESIGNED 2026-08-04. §14.1 STAYS (A7 holds)** | B1 | ⭐ `Resources/palm_anchor.py` + `analysis/verify_palm_anchor.py` (18 golden vectors). Results: **§16.11**. Built 3D-native (metric 3-vector offset in the palm frame) so the Z retrofit is ONE function; the reduction to the 2D form is proven to **6.36e-14 px**. ⚠⚠ **LIVE: a 27x REGRESSION on the very defect it targets** — still-hand cube max **11.3 px (§14.1) -> 307.5 px (palm anchor)** on a 143 s take that is 47% still frames. ⭐ **CAUSE, diagnosed in one run: the ORIENTATION channel, not the scale.** On frames where the palm CENTROID moved <0.5 px the palm QUATERNION still moved p95 **21.9°**, max **144.2°** (§0.18's defect). A 144° swing on a 60 px lever throws the cube ~114 px. **§14.1 is structurally immune because it never reads orientation at all** — a property nobody had noticed was load-bearing. ⭐ **DURABLE RESULT: an anchor's robustness is about WHICH CHANNELS IT COUPLES TO, not which landmarks it reads.** §16.10 was right that §14.1 couples to the noisiest LANDMARKS; this fix coupled to the noisiest CHANNEL, which is worse. **One cheap untested variant**: feed it the ALREADY-FILTERED orientation (`_predictive_filter_step`, which production already computes) instead of the raw quaternion. ⚠ Do not assume it works — §0.13.2 says most large jumps occur in WELL-observed frames. |
