...
| B7 | **Confirmation gate (selective fixed-lag smoother)** | perception | **BUILT + MEASURED 2026-08-04 — FAILS 2 of 4 criteria, UNWIRED** | B1 | ⭐ **Result: `GESTURE_PIPELINE_SPEC.md` §16.7.** Third measured failure of causal outlier gating. ⭐ **The half that WORKS and should be reused**: deferring the decision to F+L *does* separate — of the flags raised, the `pred` verdict discards **9.3% at a reversal vs 24.5% elsewhere (0.38×)**. ⚠ **The inherited m4 out-and-back shape scores 1.13×, i.e. NOTHING** — it was built for a 2D palm centroid, and on a SIGNED SCALAR a reversal comes back *through* the value it started from. **FAILS criterion 1** (discard ratio 9.44×, target ≤1.5×) and **criterion 3** (jitter max 0.0695 → 0.4971). Passes criterion 2 (89.5% outlier / 3.4% real). Latency **83 ms** at L=2. ⚠ Two counter-facts the ratio hides: absolute reversal harm fell **2.8×** (11.65% → 4.15%), and the reversal LABELS are contaminated because a teleport also produces velocity sign changes — so §16.6's 7.43× is an upper bound too. **Do not open a fourth variant** without a new signal (second camera, or 3.2's RTS smoothing). |
