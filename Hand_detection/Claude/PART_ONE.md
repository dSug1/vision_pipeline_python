...
| B4 | **3.3 A/B — three arms, using blocks** | decision | queued | B1 (B3 optional) | The A7 gate on §14.1. Arms: **§14.1** (5 fingertips + 4 MCPs, incumbent) vs **M8a** (frozen palm-frame offset) vs **blocks**. Replay `Position_during_rotation/translation_pivot_*.json` (360 frames each; carries `pixel_landmarks`, `world_landmarks` and `cubes.{owner,center,size}`, recorded through the REAL snap/translate logic so grab events are genuine). ⚠ **There is NO independent ground truth for cube position** — the recorded `center` is §14.1's own output — so judge on PROPERTIES: no-pop at grab, jitter while still, **yaw-sink (T4, §14.1 measures −0.25)**, **cross-plane excursion (N12)**, **teleport response (T3, `jump_test4`)** |
...
| B7 | **Confirmation gate (selective fixed-lag smoother)** | perception | **NEXT — spec written, not started** | B1 | ⭐ **Full brief: `Claude/BUILD_PREDICTION_GATE.md`.** Owner design, 2026-08-04: do not decide at frame F — **flag** it, output the prediction, and let **F+1…F+L** disambiguate. Coherent ⇒ genuine direction change (blend back to truth, feed the frames to the fit); returned to the prior trajectory ⇒ F was an outlier (the predictions already shown were correct, **no artifact at all**). **Latency is paid only at reversals and outliers, never in the common case.** ⭐ **Why this is not a third attempt at the same thing**: the out-and-back discriminant it uses is the one already serving as GROUND TRUTH in this project's analyses, where it separates **7.9% teleport vs 80.2% real movement** — the mechanism is known to work; the open question is how much lag it needs. Sweep **L ∈ {2,3,4,6}**. ⚠ **It will NOT fix edge-on or back-of-hand** — those are *sustained* errors, so F…F+L agree coherently in the wrong place and get accepted. ⚠ Teleports longer than L are accepted; the documented one lasts "a few frames". ⚠ L=2 is ~83 ms against a ~75 ms perceptibility threshold — **report latency in ms** |
| B8 | **Optimise the quadratic (weights / window / order)** | perception | **NEXT — spec written, not started** | B1 | **Brief: `BUILD_PREDICTION_GATE.md` §2.** ⚠ **A SEPARATE LEVER from B7 — do not conflate.** B8 improves how accurately the next value is predicted; B7 decides when to trust the measurement. Optimising weights alone would improve jitter and leave B3″'s 7.43× reversal ratio roughly intact — that was measured. ⚠ **The fit is currently UNWEIGHTED** (7 frames, all equal), which is the most obviously wrong part; recent frames should count more. Sweep weighting × window × order × `ACCEL_UNCERTAINTY`. **Judged on prediction error, and per S1 it must beat zero-velocity AND constant-velocity baselines at every horizon** |
