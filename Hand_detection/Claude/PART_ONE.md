...
| B4 | **Anchor + rotation A/B (blocks)** | decision | ⭐ **ARM B and HORN ROTATION both MEASURED-WINNING on 7 purpose-built takes, BUILT, NOT PORTED — awaiting a live look (A7)** | B1 | ⭐ **Results: §16.14 (anchor), §16.15 (rotation).** Modules: `Resources/palm_anchor.Arm2D` (27 golden vectors), `Resources/palm_rotation.py` (25). ⭐ **ARM B kills the SINK on every axis** — yaw 0.000, pitch **−0.000**, depth −0.001, back 0.000, against §14.1's −0.656 / **−0.807** / −0.589 / −0.083. Cost: p95 jitter +30-70% (unchanged on depth); **worst STILL-HAND step does NOT degrade** (pitch 4.81→4.64). ⭐ **HORN rotation cuts the held cube's worst ORIENTATION step 39.94°→9.64° on pitch and 58.86°→8.40° at back-of-hand** (still-hand 36.54°→3.48°, a 10x cut) — in the two bands §0.18 calls a sensor floor. Use `ref` (drift-free), not `ff` (drift unmeasured). ⚠⚠ **§16.11's 3D-native anchor is OVERTURNED**: same idea, pitch p95 27.80 vs arm B's 8.11, because it rides the 3D palm frame which DEGENERATES at edge-on. ⭐ **For the anchor, staying in 2D is a shield, not a limitation.** ⚠ Arm C (no scale) is wrong: yaw −0.745, depth −0.873 — the scale term is what decouples the sink. ⚠ Horn not SVD: a quaternion cannot express a reflection, so the §13.6.1 chirality bug is designed out rather than guarded. **NEXT: live look via `debug_prediction.bat` (top row §14.1, bottom ARM B; `--rotation horn`). A7 holds until the owner accepts.** ⭐⭐ **NEXT SESSION: `Claude/HANDOFF_ANCHOR_ROTATION.md`** — the owner runs a SIX-ARM live session (`debug_prediction.bat --arms 6`), `analysis/b4_six_arm_verdict.py` scores all six from the LIVE tracks (no replay), and the winner is wired into the debug tool AND production. **Decision rule fixed in advance: lowest SINK on the pitch take**, subject to still-hand and orientation not regressing, and the owner's eye in free play. ⚠ Production is `Resources/HandsTriggeredActions.py`, which carries its OWN copy of the snap/translate logic — both must change together (§13.6.1). ⚠ Z-axis + web-port implications of the 2D anchor are recorded in §16.16 and in `palm_anchor.Arm2D`'s docstring. |
...
| B7 | **Confirmation gate (selective fixed-lag smoother)** | perception | ⛔ **PARKED by owner 2026-08-04 — built, measured (corpus + 2 live takes), CLEARED of its blockers, then DECLINED as not visible enough to justify a layer** | B1 | ⭐ **Results: §16.7 (corpus), §16.9 (live), §16.9.1 (the park).** ⚠ **BOTH failing criteria were measured at the wrong level and both were overturned.** Criterion 3 was judged on PALM channels (the corpus has no cube); on the CUBE the held-cube worst step falls **156.5 → 109.6 px** and the worst still-hand step **73.2 → 38.7 px**. Criterion 1 (reversal-discard ratio) was **RETRACTED as a disqualifier** after the owner challenged it: measured directly, the gated cube turns on the **SAME FRAME** as the raw cube (lag p50 = p90 = **0 ms** over 1671 direction changes) — it was a B3''-era proxy for a cascading mechanism B7 does not have. **Owner's ground for parking is a product one and the data supports it**: the improvement is real but not VISIBLE, against ~89 ms hold per flag, ~4% of grabs delayed, and a per-hand predictive layer that produced two of its own defects in one session. ⭐ **Revive config, do not re-derive: `reject_z=4.0`, L=2, verdict `pred`, coast `hold`, blend 3, order-1 exp hl=2 fit.** ⚠ Nothing is wired; `debug_prediction.bat` + `analysis/b7_live_ab.py` + 2 live takes on E: are all still runnable. ⚠ **Do not open a fourth attempt** — 1.6, B3'' and B7 agree: DR-1 already removed the teleports, and what remains is sustained (a sensor floor) or invisible. |
