...
| B4 | **3.3 A/B — three arms, using blocks** | decision | queued | B1 (B3 optional) | The A7 gate on §14.1. Arms: **§14.1** (5 fingertips + 4 MCPs, incumbent) vs **M8a** (frozen palm-frame offset) vs **blocks**. Replay `Position_during_rotation/translation_pivot_*.json` (360 frames each; carries `pixel_landmarks`, `world_landmarks` and `cubes.{owner,center,size}`, recorded through the REAL snap/translate logic so grab events are genuine). ⚠ **There is NO independent ground truth for cube position** — the recorded `center` is §14.1's own output — so judge on PROPERTIES: no-pop at grab, jitter while still, **yaw-sink (T4, §14.1 measures −0.25)**, **cross-plane excursion (N12)**, **teleport response (T3, `jump_test4`)** ⭐⭐ **RE-POINTED 2026-08-04 by §16.10, and this is now the most valuable open item in Phase B.** Measured on a 450 s live take: **a perfectly still palm still moves the cube.** With the 5 palm landmarks moving <0.5 px, the 9-landmark translation anchor moves p50 **0.570 px**, p95 1.701, **max 17.51 px** — an amplification of **1.66x**. Cause: the frozen weights include the **5 FINGERTIPS**, which carry a median **27%** of the anchor even when the cube is grabbed at the palm centroid (p95 42%), and those are the 13-32% CV landmarks (§0.2) against the palm's 2.76 mm rigidity. ⭐ **Arm B REMOVES a layer rather than adding one** — the palm position (5 landmarks) and palm frame (4 landmarks) are already computed every frame, so anchoring to them deletes the fingertip path entirely. It is the only arm that decoupled BOTH axes (|r| 0.005 / 0.001 vs §14.1's 0.323 / 0.822, §16.5). ⚠ Independent of B7 and unaffected by its parking. ⚠ §14.1 must not be modified before this runs (A7). |
