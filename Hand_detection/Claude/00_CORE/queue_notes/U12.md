...
## Appended 2026-08-26 — ⚠ THE FIRST DECLARED DEPTHS, AND WHY THEY DO NOT YET CARRY

⛔⛔ **START HERE. The owner, the same day, on the takes these numbers come from:**

> *"the distance to the camera was not very reliable and the hand very likely
> moved during the takes."*

⛔ **THAT LARGELY GUTS THIS SECTION, AND IT IS RECORDED RATHER THAN QUIETLY
DROPPED.** A "scale bias at depth Y" is a statement about a distance that was set
by tape once and then held by eye through seven poses. If the arm drifted — and
the owner says it likely did — the numbers below measure the drift as much as the
estimator.

⚠ **They are kept because the direction may still be informative and because
re-deriving them from the same takes later would repeat the mistake.** They are
**not** a basis for any correction. ⭐ The ratio table these takes exist for is
unaffected: foreshortening ratios are scale-free, so a wandering hand does not
touch them. It is only the DEPTH story that needs a held distance, and it did not
have one.



The `T6` ratio-table takes carry `--declared-depth-m`, measured with a tape, lens
front to palm centre with the palm square. ⭐ **Every depth number this project
has ever quoted — including the 0.497 m working median and everything
`GRAB_Z_TOLERANCE_M` was sized around — came from the estimator, i.e. from an
assumed 60° HFOV and anthropometric medians. Nothing had been checked against a
ruler until now.**

**At the 0° hold** (palm square, unforeshortened — ⛔ *not* the take median the
tool prints; see `T6` for why that number is contaminated):

| declared | yaw | pitch |
|---|---|---|
| 0.35 m | **1.059** | *not declared* |
| 0.50 m | **0.996** | **0.941** |
| 0.70 m | **0.962** | **0.866** |

### What can and cannot be concluded

✅ **One thing survives**: a tape has now been pointed at this pipeline for the
first time, and the estimator is **in the right ballpark** — same order, right
sign, no factor-of-two surprise. Every depth this project has quoted came from the
estimator against an assumed 60° HFOV and anthropometric medians; that it is not
wildly wrong is worth knowing and was not known before.

⛔ **Everything finer than "ballpark" does not survive caveat zero.** The earlier
version of this section read a **falling ratio with distance** off these six
points and observed that neither a scale bias nor a wrong `CAMERA_HFOV_DEG`
predicts one, while an **additive reference offset** does. ⚠ **A hand drifting
during the takes predicts one too**, and it is by far the cheaper explanation.
The apparent trend is not separable from the operator.

⛔ **Do not fit anything to these six points.** Three distances per axis, set by
tape then held by eye, against a two-parameter model: such a fit succeeds by
arithmetic. At 0.35 m a 2 cm wobble is already 6%, which is the size of the whole
effect at the near end.

### ⭐ What `U12` actually needs, and it is not more of this

⛔ **A held distance, not a measured one.** The gap that matters is not the tape's
precision — it is that nothing kept the hand there. A useful depth take needs the
hand **braced** (against a surface, a stand, a doorframe) so the distance is a
constant rather than an intention, and it needs only a **single 0° hold**, not a
rotation sweep — rotation is exactly what invites the arm to drift.

⭐ Then, and only then, the reference-offset question is worth asking: measure to a
**deliberately different reference** (the camera body's back edge instead of the
lens front) and see whether the implied offset moves by exactly the amount the
reference moved. If it does, it is geometry and `U12` gets a constant to subtract.

⚠ **Until then `U12` adopts no correction.** `GRAB_Z_TOLERANCE_M = 0.15 m` absorbs
all of this and costs precision for everyone; collapsing it is the prize, but
collapsing it onto a fit of six unheld points would be worse than leaving it
generous.
