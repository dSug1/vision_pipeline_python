...
## Appended 2026-08-26 — ⭐⭐ THE FIRST DECLARED DEPTHS IN THE CORPUS
...
✅ **The estimator is roughly right at working distances** — within about ±6% on
yaw across the whole 0.35–0.70 m range. That is a genuinely reassuring result and
it is new.

⚠ **The ratio FALLS with distance on both axes**, and **pitch reads consistently
below yaw at the same distance**. ⛔ Neither of `U12`'s two candidate corrections
predicts a falling ratio: a per-user **scale bias** and a wrong
**`CAMERA_HFOV_DEG`** both produce a *constant* ratio at every distance. What does
produce one is an **ADDITIVE offset** — the estimator measuring from a different
origin than the tape did (optical centre vs lens front, or the estimator's palm
reference vs the palm centre that was measured to).

⛔⛔ **THIS IS A QUESTION, NOT A FINDING, AND IT MUST NOT BE FITTED YET.** Three
distances per axis, set by tape and then held by eye, against a two-parameter
model: such a fit succeeds by arithmetic rather than by evidence. At 0.35 m a 2 cm
wobble is already 6%, which is the size of the whole effect at the near end. This
project has an overturned refutation (§2.0.9) and a still-unexplained 3× pitch
anomaly, both from reading trends off small samples.

### ⭐ The cheap experiment that would settle it — and it needs no re-recording

Measure to a **deliberately different reference** at one distance — say the camera
body's back edge instead of the lens front — and re-take a single 0° hold. If the
implied offset moves by **exactly** the amount the reference moved, it is geometry
and `U12` gets a constant to subtract. If it does not, something else is going on
and the additive story is wrong.

⚠ **Until then `U12` should not adopt a correction.** The current
`GRAB_Z_TOLERANCE_M = 0.15 m` absorbs all of this and costs precision for
everyone; collapsing it is the prize, but collapsing it onto a two-parameter fit
of six hand-held points would be worse than leaving it generous.
