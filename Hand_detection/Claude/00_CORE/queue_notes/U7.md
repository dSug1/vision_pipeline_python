# `U7` — ⛔⛔ The handedness LABEL is wrong 10.8% of the time, and every chirality-sensitive rule inverts on it
...
>
> **Status when this file was created (2026-08-25):** ✅✅ SHIPPED AND OWNER-ACCEPTED LIVE 2026-08-22 ("fix is working. I believe this is good to ship."), and re-confirmed live in production 2026-08-23. ⛔ BUT THE SPECIFIED ACCEPTANCE TEST STILL HAS NOT RUN
...
## Appended 2026-08-25 — ✅✅✅ THE ACCEPTANCE TAKE RAN. U7 IS CLOSED.

`2026-08-25_171814_known_right_reentry_acceptance` — **physical RIGHT hand only,
declared in `meta.known_hand` BEFORE the first frame**, 1925 frames / 1127
single-hand, 18.2 fps, with repeated full re-entries and both facings.
...
⭐ **The declaration is corroborated, not just asserted.** The retracted 08-23
attempt dropped **540** multi-hand frames (64% of its hand frames); this take
dropped **8** (0.7%). That is two hands versus one, visible in the coverage line
without anyone having to be believed.

⭐ **And those 8 frames are not a second hand either** — the two detections sit
**0.10–1.00 palm widths apart**, i.e. on top of each other. MediaPipe
duplicate-detected ONE hand and gave it both labels: fresh live evidence for
`N9` / spec §0.4, on a take where the physical truth is known.

⛔⛔ **AN INSTRUMENT DEFECT WAS FOUND AND FIXED WHILE DOING THIS, and it is the
reusable part.** `analysis/u7_geometric_chirality.py` discovers sessions by
matching `known_` in the name, and `declared()` fell through to the NAME when
`meta.known_hand` was absent. The retracted take is named
`u7_acceptance_known_right` — which still contains `known_right` — so **a take
whose ground truth is recorded as FALSE (`ground_truth_valid: false`) was being
scored as if it were true**, and its 302 frames sat inside every corpus figure
the row has ever quoted. Fixed: `declared()` now returns undeclared for any
session carrying `ground_truth_valid: false` or `known_hand_RETRACTED`.
⚠ It moved the corpus numbers: 95.5%/98.8% → **97.1%/99.2%**.
