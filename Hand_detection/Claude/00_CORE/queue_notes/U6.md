...
## Appended 2026-08-25 — ⚠ RENDERER PARITY IS UNGUARDED

The debug tool lost its white snap-highlight on a held object for two days
(`febd3fa` removed it as collateral) while **production kept its equivalent**.
`parity_replay.py` reported **NO DIVERGENCE** the whole time and was **correct**:
it compares **gesture logic**, not drawing.

⭐ So U6's mechanical guard has a **hole with a definite shape**: the two tools can
diverge in anything the replay does not evaluate — what is DRAWN, and what the HUD
shows. Every automated check was green (26/26 suites, 96 handinput, 51 hardening,
parity clean) while a visible difference persisted. **The owner found it by looking
at the screen.**

⚠ **Not yet a queue row — owner's call.** The cheap version would be a fixture that
renders one frame from each tool for an identical `CubeState` and compares the two
images; the honest question first is whether that is worth its weight, given that a
live look already catches this class and is required before anything ships.
