...
>
> **STATUS** · ✅✅ **SHIPPED 2026-08-28** — built and live-confirmed in BOTH tools · **SUB** · 3D · **KIND** · feature · **DEP** · `AS1`
...
⚠⚠ **THE ROW'S OWN ACCEPTANCE METRIC IS STILL UNMEASURED**, and shipping does not close it: `snap/break transitions per minute` on a recorded take. **No recording of an assembly session exists** — the production run that shipped this row was not recorded, and the recorders do not carry mate state, so the metric cannot be produced from it without RE-DERIVING what the tool already knew (`METHOD`: *record what ran; never re-derive it*). ✅ The snap radius and angle are settled live at `snap 150 %`; ⚠ **the PREVIEW radius is still unsettled**.
