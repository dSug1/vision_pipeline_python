...
| [RB3](queue_notes/RB3.md) | Orientation — Horn in the corrected frame | HAND | perception | ⚠ **BUILT 2026-08-29, HALF-CLOSED** — `Resources/hand_orientation.py`, Horn in the corrected frame with **nothing wrapped around it**. ✅ The algebraic invariants pass: translation produces no rotation, composition holds, every output canonical (`w≥0`), mirroring the hand mirrors the rotation. ⚠⚠ **The composition check was WRONG FIRST and the TEST was at fault**: it asserted composition was EXACT and failed by 0.191° — **the palm is not rigid**, Horn leaves a measured 2.5–8.0 mm residual on a 91.5 mm palm, so the two halves cannot compose exactly. Tolerance now derived from that residual, with a wrong-order control proving it still bites. ⛔ **NOT CLOSED: the TWO-HAND take is owed.** A rotation is a property of the WORLD, so both hands doing the SAME motion must report the SAME sign — and **every take in this project's rotation work is single-hand**, so the invariant that would catch a chirality leak into orientation has never been tested. ⭐ Take specified and recorder ready (`rb3_two_hands_axes`, same-direction not mirrored, screen-relative prompts, declared directions in `meta.json`, OPERATOR-PACED steps) | RB2 |
