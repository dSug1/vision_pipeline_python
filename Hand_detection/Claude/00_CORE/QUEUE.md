...
⭐⭐ **`V1` WAS BUILT 2026-08-28 AND IS WAITING ON A LIVE LOOK** — the camera mount.
The owner reported yaw, pitch and z-translation all reading backwards; the cause is
that the shipped build takes its **mirror** from one camera mounting and its
**depth** from the other, which is a reflection rather than any viewpoint. ⭐ The
diagnosis predicted the symptom: the fix reverses yaw and pitch and leaves **roll**
exactly alone, and roll is the axis the owner did not name. **Default is `legacy`
(today's build, bit-for-bit); run `CAMERA_MOUNT=facing_user` to try it.**
⛔ Nothing is accepted until the owner has seen it in BOTH tools.
...
⚠ **OWED: the production live look** — settled in the debug tool only.
...
constants and has not been run with them.

Also ready, with nothing blocking it:
