...
| `t3_relabel_threshold.py` | ⭐ **where T3's 0.5-palm-width threshold comes from.** Candidate displacement is **median 0.11 pw, 86% inside 0.5**, then it breaks — the cluster IS the evidence that these are the same hand one frame later, and the threshold sits at its knee rather than where it would catch the most events. Also measures the guard's cost (84 of 141 candidates blocked, deliberately) and applies the shipped rule: **49 of 236 vanish-while-held moments keep their cube** |
| `verify_hand_ownership.py` | ⭐ **GOLDEN VECTORS for `hand_ownership.py` (T3)**. §3 is the load-bearing one: this module's failure mode is not a dropped cube but a cube TAKEN from the hand holding it, so the busy-slot guard is tested directly. §2 pins the measured threshold as a boundary and proves it is scale-free. ⚠ Its port-contract scan uses `tokenize` — the first version sliced off the leading docstring and false-positived on the phrase "transfer time." in a class docstring |
